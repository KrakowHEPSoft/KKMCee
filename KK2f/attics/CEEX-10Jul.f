...
*
      ...
      INTEGER               i,j,k,l,n,j1,j2,last,loop,loop2,Hel,Hel1,Hel2,GPS_faja,faja1
...
*   implicit Bose Einstein symmetrization inside HiiPlus, HffPlus
     ...
     $                     *( m_isr(j2)*Sini(Hel2,j2) + (1-m_isr(j2))*Sfin(Hel2,j2) ) !
                  ...
                  IF(     (m_isr(j1) .EQ. 1) .AND. (m_isr(j2) .EQ. 1) ) THEN ! ini-ini
                     ...
                     faja1=GPS_faja(-1) 
!                  IF (NPHOT.GT.-2) THEN
                   IF (ABS(ph1(1))+ABS(ph1(2)).GT.0.1.and.ABS(ph2(1))+ABS(ph2(2)).GT.0.1) THEN
                     ...
                     faja1=GPS_faja(-1)
                     CALL GPS_HiiPlusW(2,Cfact2,KFi,KFf,PX,p1,m1,p2,m2,p3,m3,p4,m4,Hel1,ph1,Hel2,ph2,mph,m_AmpExpo2) 
                     faja1=GPS_faja(-1) 
                     CALL GPS_HiiPlusW(2,Cfact2,KFi,KFf,PX,p1,m1,p2,m2,p3,m3,p4,m4,Hel1,ph1,Hel2,ph2,mph,m_AmpExpo2p) 
                     faja1=GPS_faja(-1)
                   ELSE
                     CALL GPS_HiiPlusW(1,Cfact2,KFi,KFf,PX,p1,m1,p2,m2,p3,m3,p4,m4,Hel1,ph1,Hel2,ph2,mph,m_AmpExpo2) 
                     faja1=GPS_faja(-1) 
                     CALL GPS_HiiPlusW(1,Cfact2,KFi,KFf,PX,p1,m1,p2,m2,p3,m3,p4,m4,Hel1,ph1,Hel2,ph2,mph,m_AmpExpo2p) 
                   ENDIF
...
!                    IF ((pr1vx/pr1v.LT.p1(4)/phv(4).or.pr2vx/pr2v.LT.p1(4)/phv(4)).and.ABS(Csum3).GT.0D0) write (*,*) 'kasujem'
                    IF (pr1vx/pr1v.LT.p1(4)/phv(4)) Csum3=0D0
                    IF (pr2vx/pr2v.LT.p1(4)/phv(4)) Csum3=0D0

!                 IF (ABS(Csum3).GT.0D0)  write (*,*) 'eee tam'
...
*/////////////////////////////////////////////////////////////////////////////////////
      ...
      INTEGER           ICNT,nin,GPS_faja,faja1,I9B,ICOL,ICOL1,NICOL,Level
*------------------------------------------------------------------------------------------
      IF     (Level.EQ.0) THEN
        ICOL =1
        ICOL1=0
...
!       numerical stability not yet checked, problems like with single brem. possible.
        ...
        I9X=NICOL   ! first finite upperline U, second for cancel      >  ggchkok
        I9Y=NICOL   ! first finite upperline V, second for cancel      >  ggchkok
        I9Z=NICOL   ! second finite upperline U, second for cancel     >  ggchkok
        I9T=NICOL   ! second finite upperline V, second for cancel     >  ggchkok
        IVI=NICOL   ! simplified IV i.e. 'true double infrared'        >  ggchkok
        IV2=ICOL1 ! ICOL1   ! double photon rest (two from 1 leg) k1*k2 ....   >  ggchkok
        IV1=ICOL1 ! ICOL1   ! double photon rest (two from 1 leg) k1*k2 ....   >  ggchkok
...
! section for test. To activate replace GPS_faja(-1) with GPS_faja(1)
!      IF ((PSI(4)-1D-5.LT.PSF(4)).AND.     !two photons
      IF ((PSI(4)-1D0.GT.PSF(4)).AND.       ! three at least
     $    (ABS(PH1o(1))+ABS(PH1o(2)).GT.1D0) .AND.
     $    (ABS(PH2o(1))+ABS(PH2o(2)).GT.1D0) ) THEN

          IF(HEL1.EQ.1.AND.HEL2.EQ.1.AND. IX.EQ.1) GOTO 23 
          IF(HEL1.EQ.1.AND.HEL2.EQ.2.AND. IY.EQ.1) GOTO 23
          IF(HEL1.EQ.2.AND.HEL2.EQ.1.AND. IZ.EQ.1) GOTO 23
          IF(HEL1.EQ.2.AND.HEL2.EQ.2.AND. IT.EQ.1) GOTO 23
          faja1=GPS_faja(-1)
 23       continue
          IF(HEL1.EQ.1.AND.HEL2.EQ.1) IX=1
          IF(HEL1.EQ.1.AND.HEL2.EQ.2) IY=1
          IF(HEL1.EQ.2.AND.HEL2.EQ.1) IZ=1
          IF(HEL1.EQ.2.AND.HEL2.EQ.2) IT=1
      ENDIF
      IF(GPS_faja(0).EQ.1) CALL GPS_BornZero(AmpWork)
*------------------------------------------------------------------------------------------
      Y_IR=1                   ! YES, IR included
      Y_IR=0                   ! No,  IR not included
...
!      RETURN
      IF( m_KeyArb  .EQ.  0 ) THEN
         ...
         sB(1,1)  =  gI*GPS_Sof1( 1,ph1,pB)
         sB(2,1)  =  gI*GPS_Sof1( 1,ph2,pB)
         EpsDot1(1) = -gI*0.5D0*(-GPS_Sof1x( 1,ph1,pA)+GPS_Sof1x( 1,ph1,pC)  
     $                           +GPS_Sof1x( 1,ph1,pB)-GPS_Sof1x( 1,ph1,pD)-GPS_Sof1x( 1,ph1,ph2))
         EpsDot12(1)= -gI*0.5D0*(-GPS_Sof1x( 1,ph1,pA)+GPS_Sof1x( 1,ph1,pC)+GPS_Sof1x( 1,ph1,ph2) 
     $                           +GPS_Sof1x( 1,ph1,pB)-GPS_Sof1x( 1,ph1,pD))
         EpsDot2(1) = -gI*0.5D0*(-GPS_Sof1x( 1,ph2,pA)+GPS_Sof1x( 1,ph2,pC)  
     $                           +GPS_Sof1x( 1,ph2,pB)-GPS_Sof1x( 1,ph2,pD)-GPS_Sof1x( 1,ph2,ph1))
         EpsDot21(1)= -gI*0.5D0*(-GPS_Sof1x( 1,ph2,pA)+GPS_Sof1x( 1,ph2,pC)+GPS_Sof1x( 1,ph2,ph1) 
     $                           +GPS_Sof1x( 1,ph2,pB)-GPS_Sof1x( 1,ph2,pD))
      ELSE
         ...
         sB(1,1)  =  gI*GPS_Sof1b( 1,ph1,pB,mB)
         sB(2,1)  =  gI*GPS_Sof1b( 1,ph2,pB,mB)
         EpsDot1(1) = -gI*0.5D0*(-GPS_Sof1bx( 1,ph1,pA,mA)+GPS_Sof1bx( 1,ph1,pC,mC)  
     $                           +GPS_Sof1bx( 1,ph1,pB,mB)-GPS_Sof1bx( 1,ph1,pD,mD)-GPS_Sof1bx( 1,ph1,ph2,mph))
         EpsDot12(1)= -gI*0.5D0*(-GPS_Sof1bx( 1,ph1,pA,mA)+GPS_Sof1bx( 1,ph1,pC,mC)+GPS_Sof1bx( 1,ph1,ph2,mph) 
     $                           +GPS_Sof1bx( 1,ph1,pB,mB)-GPS_Sof1bx( 1,ph1,pD,mD))
         EpsDot2(1) = -gI*0.5D0*(-GPS_Sof1bx( 1,ph2,pA,mA)+GPS_Sof1bx( 1,ph2,pC,mC)  
     $                           +GPS_Sof1bx( 1,ph2,pB,mB)-GPS_Sof1bx( 1,ph2,pD,mD)-GPS_Sof1bx( 1,ph2,ph1,mph))
         EpsDot21(1)= -gI*0.5D0*(-GPS_Sof1bx( 1,ph2,pA,mA)+GPS_Sof1bx( 1,ph2,pC,mC)+GPS_Sof1bx( 1,ph2,ph1,mph) 
...
C
     ...
     $                                                                         )               
      ...
      ENDDO
       ...
       IF(GPS_faja(0).eq.0) return

       WRITE(*,*) ' '
       WRITE(*,*) ' ======================== '
       WRITE(*,*) ' HEL1= ',HEL1,' HEL2= ',HEL2
       WRITE(*,*) ' '
       ...
       WRITE(*,*) 'PSI=',PSI 
       WRITE(*,*) 'PSF=',PSF 
C>>>>       write(*,*) 'Test of GPS_Make_eps(ph1,Sig,eps1)'
C>>>>       write(*,*) 'we calculate ,EpsDotXX from old routine and later usingGPS_Make_eps'
C>>>>      write(*,*) '21=',EpsDot21(Hel2),'  ',-eps2pA+eps2pC+eps2p1+eps2pB-eps2pD
C>>>>      write(*,*) '2 =',EpsDot2 (Hel2),'  ',-eps2pA+eps2pC-eps2p1+eps2pB-eps2pD
C>>>>      write(*,*) '1 =',EpsDot1 (Hel1),'  ',-eps1pA+eps1pC-eps1p2+eps1pB-eps1pD
C>>>>      write(*,*) '12=',EpsDot12(Hel1),'  ',-eps1pA+eps1pC+eps1p2+eps1pB-eps1pD
CXXX>                  write(*,*) 'desperate test to find ANY cancelations warning: simplifications in m.e.!? '
CXXX>                  write(*,*) '1=', sA(1,Hel1)+sB(1,Hel1)
CXXX>                  write(*,*) 't,t2',t,t2
CXXX>                  write(*,*) 'eps2=',eps2      
CXXX>                  write(*,*) 'eps2*ph2=',eps2(4)*ph2(4)-eps2(3)*ph2(3)-eps2(2)*ph2(2)-eps2(1)*ph2(1)

CXXX>                  write(*,*) '2=', sA(2,Hel2)+sB(2,Hel2)
CXXX>                  write(*,*) 'AB=', sA(2,Hel2),sB(2,Hel2)
CXXX>                  write(*,*) 'up=',sA(2,Hel2)/t2+sB(2,Hel2)/t
CXXX>                  write(*,*) 'int=',EpsDot2(Hel2)/t/t2,EpsDot2(Hel2),-2*eps2pA+2*eps2pC
CXXX>                  write(*,*) 'sum=',sA(2,Hel2)/t2+sB(2,Hel2)/t+EpsDot2(Hel2)/t/t2
CXXX>                  write(*,*) 'cpf0=',cpf0, abs(cpf0),WVpi0
CXXX>                  write(*,*) 'AB=',
CXXX>     $                  ( sA(1,Hel1)*sA(2,Hel2)*Fprop1B                       *IVI !
CXXX>     $                 +                             +sA(1,Hel1)*sB(2,Hel2)*(CPF1/CPF0-1d0)               *IVI
CXXX>     $                 +                             +sB(1,Hel1)*sA(2,Hel2)*(CPF2/CPF0-1d0)              *IVI
CXXX>     $                 +                             +sB(1,Hel1)*sB(2,Hel2)*Fprop2B                       *IVI
CXXX>     $                 +                             +sA(1,Hel1)*CPF1*CPF12/CPF0*EpsDot21(Hel2)             *IVI! terms due to diag WWgamma
CXXX>     $                 +                             +sA(2,Hel2)*CPF2*CPF12/CPF0*EpsDot12(Hel1)          *IVI
CXXX>     $                 +                             +sB(1,Hel1)*CPF*CPF2/CPF0*EpsDot2(Hel2)             *IVI
CXXX>     $                 +                             +sB(2,Hel2)*CPF*CPF1/CPF0*EpsDot1(Hel1)             *IVI
CXXX>     $                 +                             +CPF*CPF1*CPF12/CPF0*EpsDot1(Hel1)*EpsDot21(Hel2)   *IVI
CXXX>     $                 +                             +CPF*CPF2*CPF12/CPF0*EpsDot2(Hel2)*EpsDot12(Hel1)   *IVI
CXXX>     $                                                                                      ) ,


CXXX>     $                 (-m_e_QED**2)*CPF*CPF12/CPF0   !+CNorm*(-1D0) *DCMPLX(0,-m_e_QED**2)*CPF*CPF12*WVpi0
CXXX>     $                 *2*eps1D2*IVI

      Call GPS_BPrint(0,'AmpWplus  ',AmpWork)
! 23   CONTINUE
      nin=nin+1
      if (nin.eq.40) stop
      IF(IX.EQ.1.AND.IY.EQ.1.AND.IZ.EQ.1.AND.IT.EQ.1) stop

      END                       ! GPS_HiiPlusW

      Integer function GPS_faja(jak)
      implicit none
      integer faja0,jak
      save faja0
      if(jak.eq.1) faja0=1
