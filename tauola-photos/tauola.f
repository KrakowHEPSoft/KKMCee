...
C boost and append photon (gamma is 22)
      ...
      CALL FILHEP3(0,1,22,NPS,NPS,0,0,PHOT,0.0,.TRUE.)
C
      RETURN
      END
...
C tau neutrino (nu_tau is 16)
      ...
      IF(ND.EQ.2.AND.IDFFIN(3,JNPI).NE.0) THEN
        ...
        CALL FILHEP3(0,1,-IDFFIN(3,JNPI)*ISGN,-IS,-IS,0,0,PNU,AM,.TRUE.)
      ELSEIF(ND.EQ.2.AND.IDFFIN(4,JNPI).NE.0) THEN
        IS=1
        CALL FILHEP(0,1, IDFFIN(4,JNPI)     ,-IS,-IS,0,0,PNU,AM,.TRUE.)
        CALL FILHEP3(0,1, IDFFIN(4,JNPI)     ,-IS,-IS,0,0,PNU,AM,.TRUE.)
...
c sigle scalar has no W
      ELSEIF(ND.EQ.1) THEN
        ...
        CALL FILHEP3(0,1,16*ISGN,-IS,-IS,0,0,PNU,AM,.TRUE.)
...
C W boson (W+ is 24)
        ...
        CALL FILHEP3(0,1,-24*ISGN,NPS,NPS,0,0,PWB,AM,.TRUE.)
      ENDIF
C
...
C        IF(KFPI.NE.111)KFPI=KFPI*ISGN
        ...
        CALL FILHEP3(0,1,KFPI,-I-IS,-I-IS,0,0,PPI,AM,.TRUE.)
      END DO
C
      RETURN
      END
