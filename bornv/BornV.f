...
* for special tests
      m_QEDmodif = 1.d0                 ! QED constant modifiction factor
...
c[[[[[[[[[[[[[[[[[[[[[[[[[
c      CALL KarLud_GetNevGen(NevGen)
c      IF(NevGen.gt.0) THEN
c      IF(NevGen.eq.1) m_vv=0.4
c      IF(NevGen.eq.2) m_vv=0.2
c      IF(NevGen.eq.3) m_vv=0.6
c      IF(NevGen.eq.4) m_vv=0.8
c      IF(NevGen.eq.5) m_vv=0.1
c      IF(NevGen.eq.6) m_vv=0.3
c      IF(NevGen.eq.7) m_vv=0.5
c      IF(NevGen.eq.8) m_vv=0.7
c      IF(NevGen.eq.9) m_vv=0.0001
cc      seed = 54217317+10000000*m_vv
cc      seed = 54217317
cc      CALL pseumar_initialize(seed, 0, 0)
c      write(*,*)     '================================================================================================='
c      write(*,*)      '$$$ BornV_RhoVesko1:  m_vv=', m_vv
c      write(m_out,*) '================================================================================================='
c      write(m_out,*) '================================================================================================='
c      write(m_out,*) '================================================================================================='
c      write(m_out,*) '================================================================================================='
c      write(m_out,*) '$$$ BornV_RhoVesko1:  m_vv=', m_vv
c      ENDIF
c]]]]]]]]]]]]]]]]]]]]]]]]]
...
***         STOP
      ENDIF
      IF ( ABS(m_QEDmodif -1d0 ) .GT. 1d-9) THEN
          m_GSW(6) = 2d0 -(2d0 - m_GSW(6) )/m_QEDmodif
      ENDIF

      m_QCDcor = m_QCDcorR(1)-1d0 ! <--- obsolete!!!
      END                        !BornV_InterpoGSW
